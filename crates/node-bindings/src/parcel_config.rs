...
impl<'de> serde::Deserialize<'de> for PipelineNode {
    ...
    D: serde::Deserializer<'de>,
  {
    ...
    // return Ok(PluginNode(node));
    // }
    Ok(PipelineNode::Spread(Spread {}))
  }
}
...
pub struct PipelinesMap {
  pub map: HashMap<String, Vec<PipelineNode>>,
}
...
impl<'de> Deserialize<'de> for PipelinesMap {
  fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
  where
    D: serde::Deserializer<'de>,
  {
    ...
    Ok(PipelinesMap(value))
  }
}

impl Hash for PipelinesMap {
  fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
    self.1.hash(state);
  }
}

// impl FromNapiValue for PipelinesMap {
...
// impl ToNapiValue for PipelinesMap {
